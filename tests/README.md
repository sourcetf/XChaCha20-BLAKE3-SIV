...
| Concurrent use | **run**: `tests/threads.rs`, and AddressSanitizer over the whole suite | The tag is derived through a cached AVX2 detection (`AtomicU8`), so 32 threads race its first call from cold and must still agree on every ciphertext and tag; the detached API is exercised in the same loop. **ThreadSanitizer is not run**: it cannot compile this crate's dev-dependencies (`regex-syntax` fails under `-Zsanitizer=thread`), so races rest on that stress test plus the shared state being one relaxed atomic and no other mutable statics. `broad_differential.py` (4000 random vectors) and `tools/mutation_check.sh` cover what a sanitizer would not. |
