...
| Checks that are not vacuous | **run**: `tools/mutation_check.sh`, plus `tools/fi_check.sh`, a seven-row fault campaign over the decision, `cargo mutants` over the same functions (with the four equivalent `&`/`|` mutants excluded and the reason recorded), and `tools/fi_instruction.sh`, which corrupts the compiled decision code one byte at a time and maps which bytes accept a forgery: 3428 faults in the default build and 4690 in the hardened one, none of them accepting. Summary: one skipped branch accepts a forgery on the default build, one skipped gate or corrupted gate value does not on the `hardened` build, replacing the computed tag is accepted by both (a pinned known limit), and a skipped in-place wipe is caught by `tests/security.rs` |
