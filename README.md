...
| ...the same, in the **opt-out** build (`default-features = false`) | **1 bit, pinned**: 44856 faults, **1** accepting, at the opcode of the decision's own arm selection (file offset of the measured site: `0x…938`, the `74 49` = `je` whose flipped twin is `jne`). One gate has nothing behind it, which is the measured argument for the default. `tools/fi_instruction.sh` pins the count at 1: a change that makes it two fails |
